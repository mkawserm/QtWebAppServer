    ...
    :HttpRequestHandler(parent)
{
    ...
    qCDebug(HttpServer_Native(),"StaticFileController: docroot=%s, encoding=%s, maxAge=%i",qPrintable(docroot),qPrintable(encoding),maxAge);
    maxCachedFileSize=settings->value("maxCachedFileSize","65536").toInt();
    cache.setMaxCost(settings->value("cacheSize","1000000").toInt());
    cacheTimeout=settings->value("cacheTime","60000").toInt();
    qCDebug(HttpServer_Native(),"StaticFileController: cache timeout=%i, size=%i",cacheTimeout,cache.maxCost());
}
...
void StaticFileController::service(HttpRequest& request, HttpResponse& response)
{
    ...
    if (entry && (cacheTimeout==0 || entry->created>now-cacheTimeout))
    {
        ...
        mutex.unlock();
        qCDebug(HttpServer_Native(),"StaticFileController: Cache hit for %s",path.data());
    ...
    else
    {
        ...
        qCDebug(HttpServer_Native(),"StaticFileController: Cache miss for %s",path.data());
        ...
        qCDebug(HttpServer_Native(),"StaticFileController: Open file %s",qPrintable(file.fileName()));
...
void StaticFileController::setContentType(QString fileName, HttpResponse& response) const
{
    ...
    {
        qCDebug(HttpServer_Native(),"StaticFileController: unknown MIME type for filename '%s'", qPrintable(fileName));
    }
}
