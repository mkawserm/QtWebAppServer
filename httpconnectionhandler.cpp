    ...
    : QThread()
{
    ...
    //NOTE: don't know why commenting out
    // the following lines solve the problem
    //of running respose from qml
    // execute signals in my own thread
    moveToThread(this);
    socket->moveToThread(this);
    readTimer.moveToThread(this);
    //requestHandler->moveToThread(this);
    //qCDebug(HttpServer_Native())<<"SOCKET:" << socket->thread();
    //qCDebug(HttpServer_Native())<<"TIMER:" << readTimer.thread();
    //qCDebug(HttpServer_Native())<<"Current:"<< QThread::currentThread();
    ...
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): constructed", this);
    this->start();
}
...
HttpConnectionHandler::~HttpConnectionHandler()
{
    quit();
    wait();
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): destroyed", this);
}
...
void HttpConnectionHandler::createSocket()
{
    // If SSL is supported and configured, then create an instance of QSslSocket
        ...
        if (sslConfiguration)
        {
            ...
            socket=sslSocket;
            qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): SSL is enabled", this);
            return;
        }
    #endif
...
{
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): thread started", this);
    try
    {
        exec();
    }
    ...
    delete socket;
    readTimer.stop();
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): thread stopped", this);
}


void HttpConnectionHandler::handleConnection(tSocketDescriptor socketDescriptor)
{
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): handle new connection", this);
    busy = true;
    ...
    if (!socket->setSocketDescriptor(socketDescriptor))
    {
        ...
        if (sslConfiguration)
        {
            qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): Starting encryption", this);
...
{
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): read timeout occured",this);
...
{
    qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): disconnected", this);
    socket->close();
    readTimer.stop();
    busy = false;
}
...
void HttpConnectionHandler::read()
{
    ...
    {
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): read input",this);
        #endif
        ...
        {
            readTimer.stop();
            qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): received request",this);
            ...
            qCDebug(HttpServer_Native(),"HttpConnectionHandler (%p): finished request",this);
