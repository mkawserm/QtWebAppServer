    ...
    :QObject(parent)
{
    ...
    qCDebug(HttpServer_Native(),"HttpSessionStore: Sessions expire after %i milliseconds",expirationTime);
}
...
QByteArray HttpSessionStore::getSessionId(HttpRequest& request, HttpResponse& response)
{
    ...
    if (!sessionId.isEmpty())
    {
        ...
        {
            qCDebug(HttpServer_Native(),"HttpSessionStore: received invalid session cookie with ID %s",sessionId.data());
            sessionId.clear();
        }
    }
    mutex.unlock();
...
HttpSession HttpSessionStore::getSession(HttpRequest& request, HttpResponse& response, bool allowCreate)
{
    ...
    if (allowCreate)
    {
        ...
        qCDebug(HttpServer_Native(),"HttpSessionStore: create new session with ID %s",session.getId().data());
...
void HttpSessionStore::sessionTimerEvent()
{
    ...
    while (i != sessions.end())
    {
        ...
        {
            qCDebug(HttpServer_Native(),"HttpSessionStore: session %s expired",session.getId().data());
            sessions.erase(prev);
        }
    }
    mutex.unlock();
