...
#ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpSession: created new session data with id %s",dataPtr->id.data());
#endif
    }
    else
    {
...
HttpSession::HttpSession(const HttpSession& other)
{
    ...
    if (dataPtr)
    {
        ...
        dataPtr->refCount++;
#ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpSession: refCount of %s is %i",dataPtr->id.data(),dataPtr->refCount);
#endif
        dataPtr->lock.unlock();
    }
}
...
#ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpSession: refCount of %s is %i",oldPtr->id.data(),oldPtr->refCount);
#endif
        oldPtr->lock.unlock();
        if (refCount==0)
        {
...
HttpSession::~HttpSession()
{
    if (dataPtr) {
        ...
        if (refCount==0)
        {
            qCDebug(HttpServer_Native(),"HttpSession: deleting data");
            delete dataPtr;
        }
    }
}
