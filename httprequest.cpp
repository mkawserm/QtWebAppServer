...
{
    #ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpRequest: read request");
    #endif
    ...
    {
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: collecting more parts until line break");
        #endif
        return;
    }
...
{
    #ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpRequest: read header");
    #endif
    int toRead=maxSize-currentSize+1; // allow one byte more to be able to detect overflow
    lineBuffer.append(socket->readLine(toRead));
    currentSize+=lineBuffer.size();
    ...
    if (colon>0)
    {
        ...
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: received header %s: %s",currentHeader.data(),value.data());
        #endif
    }
    else if (!newData.isEmpty())
    {
        ...
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: read additional line of header");
        #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: headers completed");
        #endif
        ...
        {
            #ifdef SUPERVERBOSE
                qCDebug(HttpServer_Native(),"HttpRequest: expect no body");
            #endif
            status=complete;
        }
        ...
        else if (!boundary.isEmpty() && expectedBodySize>maxMultiPartSize)
        {
            ...
            #ifdef SUPERVERBOSE
                qCDebug(HttpServer_Native(),"HttpRequest: expect %i bytes body",expectedBodySize);
            #endif
            status=waitForBody;
        }
    }
...
void HttpRequest::readBody(QTcpSocket* socket)
{
    ...
    if (boundary.isEmpty())
    {
        ...
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: receive body");
        #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: receiving multipart body");
        #endif
        ...
        {
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: received whole multipart body");
        #endif
            tempFile->flush();
            if (tempFile->error())
            {
...
{
    #ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpRequest: extract and decode request parameters");
    #endif
    // Get URL parameters
...
{
    #ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpRequest: extract cookies");
    #endif
    foreach(QByteArray cookieStr, headers.values("cookie"))
    {
        ...
        {
            #ifdef SUPERVERBOSE
                qCDebug(HttpServer_Native(),"HttpRequest: found cookie %s",part.data());
...
{
    qCDebug(HttpServer_Native(),"HttpRequest: parsing multipart temp file");
    tempFile->seek(0);
    bool finished=false;
    while (!tempFile->atEnd() && !finished && !tempFile->error())
    {
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: reading multpart headers");
        #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("Content-Disposition:"))
            {
                if (line.contains("form-data"))
                {
                    ...
                    #ifdef SUPERVERBOSE
                        qCDebug(HttpServer_Native(),"HttpRequest: multipart field=%s, filename=%s",fieldName.data(),fileName.data());
                    #endif
                }
                else
                {
                    qCDebug(HttpServer_Native(),"HttpRequest: ignoring unsupported content part %s",line.data());
                }
            }
        ...
        #ifdef SUPERVERBOSE
            qCDebug(HttpServer_Native(),"HttpRequest: reading multpart data");
        #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("--"+boundary))
            {
                ...
                if (fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    qCDebug(HttpServer_Native(),"HttpRequest: set parameter %s=%s",fieldName.data(),fieldValue.data());
                }
                else if (!fileName.isEmpty() && !fieldName.isEmpty())
                {
                    // last field was a file
                    #ifdef SUPERVERBOSE
                        qCDebug(HttpServer_Native(),"HttpRequest: finishing writing to uploaded file");
                    #endif
                    uploadedFile->resize(uploadedFile->size()-2);
                    uploadedFile->flush();
                    uploadedFile->seek(0);
                    parameters.insert(fieldName,fileName);
                    qCDebug(HttpServer_Native(),"HttpRequest: set parameter %s=%s",fieldName.data(),fileName.data());
                    uploadedFiles.insert(fieldName,uploadedFile);
                    qCDebug(HttpServer_Native(),"HttpRequest: uploaded file size is %i",(int) uploadedFile->size());
                }
    ...
    #ifdef SUPERVERBOSE
        qCDebug(HttpServer_Native(),"HttpRequest: finished parsing multipart temp file");
    #endif
}

HttpRequest::~HttpRequest()
{
    qCDebug(HttpServer_Native()) << "HttpRequest complete and destroying.";
