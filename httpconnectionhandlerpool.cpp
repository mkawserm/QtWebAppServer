...
HttpConnectionHandlerPool::~HttpConnectionHandlerPool()
{
    ...
    qCDebug(HttpServer_Native(),"HttpConnectionHandlerPool (%p): destroyed", this);
}
    ...
    foreach(HttpConnectionHandler* handler, pool)
    {
        if (!handler->isBusy())
        {
            if (++idleCounter > maxIdleHandlers)
            {
                ...
                qCDebug(HttpServer_Native(),"HttpConnectionHandlerPool: Removed connection handler (%p), pool size is now %i",handler,pool.size());
...
void HttpConnectionHandlerPool::loadSslConfig()
{
    ...
    if (!sslKeyFileName.isEmpty() && !sslCertFileName.isEmpty())
    {
            ...
            qCDebug(HttpServer_Native(),"HttpConnectionHandlerPool: SSL settings loaded");
         #endif
    }
}
